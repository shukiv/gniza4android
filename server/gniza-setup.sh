...
ssh-keygen -t rsa -b 2048 -f "${KEY_PATH}" -N "" -C "gniza-backup" -q
chmod 600 "${KEY_PATH}"
...
if command -v croc &>/dev/null; then
    ...
    # Send key via stdin — croc on Android can't receive named files properly
    cat "${KEY_PATH}" | croc send > "$CROC_FIFO" 2>&1 &
    CROC_PID=$!
